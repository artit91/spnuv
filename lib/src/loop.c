...
SpnHashMap *new_loop(int is_default) {
    ...
    SpnValue boolVal = spn_makebool(is_default);
    spn_hashmap_set_strkey(members, "isDefault", &boolVal);
    spn_value_release(&boolVal);
...
int loop_default(SpnValue *ret, int argc, SpnValue argv[], void *ctx) {
    ...
    *ret = (SpnValue){ .type = SPN_TYPE_HASHMAP, .v.o = default_loop };
    return 0;
}
